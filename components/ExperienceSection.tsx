...
export default function ExperienceSection({ language = 'EN' }: ExperienceSectionProps) {
  ...
  const initialId =
    freelanceExperiences[0]?.id ??
    contractorExperiences[0]?.id ??
    (experiencesByTab.freelance[0]?.id || '');
  ...
  const fallback =
    currentList[0] ||
    freelanceExperiences[0] ||
    contractorExperiences[0] || {
      id: 'empty',
      role: '',
      company: '',
      period: '',
    ...
    <main className="relative w-screen h-screen overflow-hidden">
      ...
      <div className="relative z-10 flex flex-col lg:flex-row items-center justify-center w-full h-full px-6 sm:px-10 gap-10 lg:gap-16">
        ...
        >
          ...
          <div className="inline-flex items-center rounded-full border border-white/15 bg-white/5 p-1 mb-5">
            {(['freelance', 'contractor'] as ExperienceTab[]).map(tab => {
              const isActive = tab === activeTab;
              const label = tab === 'freelance' ? copy.freelanceLabel : copy.contractorLabel;
              ...
              return (
                <button
                  ...
                  className="relative px-4 py-1.5 text-[10px] uppercase tracking-[0.2em] text-white/60"
                >
                  <motion.div
                  ...
                  <ul className="space-y-1.5 text-xs sm:text-sm text-white/70">
                    {activeExperience.highlights.map(item => (
                      <li key={item} className="flex gap-2">
                        <span className="mt-[5px] h-[3px] w-3 rounded-full bg-linear-to-r from-[#9b5cff] to-[#5cd9ff]" />
                        <span>{item}</span>
                      </li>
                    ))}
                  </ul>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {activeExperience.stack.map(tech => (
                      <span
                        key={tech}
                        className="px-3 py-1 rounded-full border border-white/15 bg-white/5 text-[10px] tracking-[0.14em] uppercase text-white/60"
                      >
                        {tech}
                      </span>
                    ))}
              ...
              return (
                ...
                >
                  ...
                  <div className="relative rounded-2xl border border-white/10 bg-[#04040a]/80 backdrop-blur-md px-4 py-4 sm:px-4 sm:py-4 shadow-[0_10px_40px_rgba(0,0,0,0.75)] overflow-hidden">
                    ...
                    <p className="text-[11px] sm:text-xs text-white/60 mb-2 line-clamp-2">
                    ...
                    <div className="flex flex-wrap gap-1.5 mt-1">
                      {experience.stack.slice(0, 3).map(tech => (
                        <span
                          key={tech}
                          className="px-2 py-0.5 rounded-full bg-white/5 text-[9px] tracking-[0.15em] uppercase text-white/50"
                        >
                          {tech}
                        </span>
                      ))}
