...
export async function autoTranslateExperience(base: {
    ...
    const mapExperiences = (
        translatedExperiences?: { role?: string; description?: string; achievements?: unknown[] }[],
    ): ExperienceItem[] => {
        return base.experiences.map((baseExp, index) => {
            const translated = Array.isArray(translatedExperiences) ? translatedExperiences[index] : undefined;
            const achievements = Array.isArray(translated?.achievements) && translated.achievements.length > 0
                ? translated.achievements.map(item => (typeof item === 'string' ? item : String(item ?? '')))
